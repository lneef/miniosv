...
//
// The DPDK-named types are plain POD at global scope so the driver code (compiled
// as C++) uses them directly. Only the structure layout is modelled; device
// registration/probing and the burst inline wrappers are out of scope.
...
// ---------------------------------------------------------------------------
// PCI probe/remove helpers (DPDK <ethdev_pci.h> equivalents). probe allocates an
// rte_eth_dev (plus its data and driver-private block), runs dev_init, and on
// success enqueues it on rte_eth_dev::store; remove reverses this.
// ---------------------------------------------------------------------------
...
struct rte_pci_driver {
  ...
  using registry_t = minidpdk::intrusive_list<rte_pci_driver, &rte_pci_driver::hook>;
