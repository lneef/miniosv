...
__inline rte_mbuf *rte_pktmbuf_alloc(rte_pktmbuf_pool *pool) {
    return pool->alloc_single();
}
const void *rte_pktmbuf_read(rte_mbuf *, uint32_t, uint32_t, uint8_t *);

// socket_id is ignored (single NUMA node under OSv); marked unused in the impl.
rte_pktmbuf_pool *rte_pktmbuf_pool_create(const char *name, unsigned n,
                                          unsigned cache_size, uint16_t priv_size,
                                          uint16_t data_room_size, int socket_id);

inline void rte_pktmbuf_attach_extbuf(rte_mbuf *m, void *buf_addr, uintptr_t iova,
